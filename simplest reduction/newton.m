function [U_it, it, criterion]=...
                 newton(tol1,it_newt_max,it_damp_max,U_ini,...
...
%   shear,bulk,lame - material parameters (constants)
...
%
  ...
  [S,DS]=constitutive_problem(E,c_bar,sin_phi,shear,bulk,lame);
...
%
  ...
  while true         
     ...
     % damping parameter
     omega=damping(it_damp_max,U_it,dU,F,f,B,WEIGHT,c_bar,sin_phi,shear,bulk,lame);
     
     % recomputation with a regularized stiffness matrix when omega=0
     if omega==0 
         r=0.01;
         K_tangent = r*K_elast+(1-r)*K_tangent;
         dU(Q) = K_tangent(Q,Q)\(f(Q)-F(Q)); 
         omega=1;
     end
