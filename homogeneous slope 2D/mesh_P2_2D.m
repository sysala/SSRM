...
function [coord,elem,elem_ed,edge_el,Q]= mesh_P2_2D(h,x1,x2,x3,y1,y2)

% =========================================================================
%
%  This function creates a triangular mesh for P2 elements and
...
%               size(elem)=(6,n_e), n_e = number of elements (triangles)
...
%
  ...
  coord_x12=linspace(0,x1+x2,2*Nx12+1);
  coord_x3=linspace(x1+x2,x1+x2+x3,2*Nx3+1);
  coord_x=[coord_x12 coord_x3(2:end)];
  
  % coordinates in x-direction (below the slope)
  coord_y1=linspace(0,y1,2*Ny1+1);
  coord_y2=linspace(y1,y1+y2,2*Ny2+1);
  coord_y=[coord_y1 coord_y2(2:end)];
  
  % sizes of arrays coord and V
  coord=zeros(2,(2*Ny1+1)*(2*Nx+1)+2*Ny2*(2*Nx12+1));
  C=zeros(2*Nx+1,2*Ny+1); n_n=0;
  ...
  for j=(2*Ny1+2):(2*Ny+1)
      x_max=x1+x2*(y1+y2-coord_y(j))/y2;
      coord_x=linspace(0,x_max,2*Nx12+1);
    for i=1:(2*Nx12+1)
         n_n = n_n+1 ;
         C(i,j) = n_n ;
         coord(:,n_n) = [coord_x(i); coord_y(j)] ;
...
%
  ...
  for  j = 1:Ny1
    for  i = 1:Nx
      n_e = n_e + 1 ;
      elem(:,n_e) = [ C(2*i-1,2*j-1); C(2*i+1,2*j-1); C(2*i-1,2*j+1); ...
