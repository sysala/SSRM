...
function [coord,elem,surf,neumann,Q]=mesh_P1_3D_n(N_h,x1,x2,x3,y1,y2,z)
...
%    neumann - a part of the array surf, where nonhomogeneous Neumann 
...
%
  ...
  c_x=x1+x2+x3-[repmat(coord_x,1,(N1_y+1)*(N_z+1)),...
         repmat(cy_x(:)',1,N_z+1) ];   
...
%
  ...
  % Face 2: y=y1 (the top of body in front of the slope - slope toe level)
  %   ordering of the nodes creating the unit square:
  ...
  % Face 4: x=x1+x2+x3 (the right face of body in front of the slope)
  %   ordering of the nodes creating the unit square:
  ...
  % Face 6: x=0 (the face left from the slope)
  %   ordering of the nodes creating the unit square:
...
%
  ...
  % nonhomogeneous Neumann boundary conditions on Face 2
  neumann=surf3;     

  % logical array indicating the nodes with the Dirichlet boundary cond.
  Q=false(3,n_n);
  Q(1,:) = (coord(1,:)>0)&(coord(2,:)>0)&(coord(1,:)<x1+x2+x3-1e-6) ;
  Q(2,:) = coord(2,:)>0 ;
  Q(3,:) = (coord(3,:)>0)&(coord(2,:)>0)&(coord(3,:)<z) ;
