function f_t = vector_traction_n_3D(ELEM_s,COORD,f_n_int,HatP_s,DHatP1_s,DHatP2_s,WF_s)
...
%                         
  ...
  COORDs3=reshape(COORD(3,ELEM_s(:)),n_p_s,n_e_s);
  ...
  COORDint3=kron(COORDs3,ones(1,n_q_s));
  
  % derivatives of the isoparametric function at integration points
  % size(J11)=size(J12)=size(J13)=size(J21)=size(J22)=size(J23)=(1,n_int_s)
  J11=sum(COORDint1.*DHatPhi1_s); 
  J12=sum(COORDint2.*DHatPhi1_s); 
  J13=sum(COORDint3.*DHatPhi1_s); 
  J21=sum(COORDint1.*DHatPhi2_s); 
  J22=sum(COORDint2.*DHatPhi2_s);
  J23=sum(COORDint3.*DHatPhi2_s);
  
  % components of the normal direction: size=(1,n_int_s)
  normal1=J12.*J23-J13.*J22;
  normal2=J13.*J21-J11.*J23;
  normal3=J11.*J22-J12.*J21;

  % transformation ratio between real and reference surfaces at each int.p.
  % size(ratio)=(1,n_int_s)
  ratio=sqrt(normal1.^2+normal2.^2+normal3.^2);

  % components of outward normal vectors at integration points
  % size(normal1)=size(normal2)=(1,n_int_s)
  normal1=normal1./ratio;
  normal2=normal2./ratio; 
  normal3=normal3./ratio; 
...
%
  ...
  vF3 = HatPhi_s.*(ones(n_p_s,1)*(WEIGHT_s.*normal3.*f_n_int)); 
