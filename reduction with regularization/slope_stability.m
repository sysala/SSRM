...
%  elements, the 7-point Gauss quadrature is used. We apply a regularization
%  method to find the safety factor. In particular, we use a fixed value
%  of the regularization parameter alpha and increase the strength 
%  reduction factor lambda. The semismmoth Newton method with damping is
%  used for any investigated value of lambda.
% 
% ======================================================================
%
...
%
% Elastic solution and the corresponding energy parameter alpha_el
%
...
%
% Computation of the SSRM factor of safety by the regularization method
%

  % input parameters for the solver with fixed alpha
  alpha=10*alpha_el;   % prescribed value of the regularization parameter
  lambda_init=0.5;     % initial lower bound of lambda
  d_lambda_init=0.1;   % initial increment of lambda 
  d_lambda_min=1e-4;   % minimal increment of lambda 
  
  [U,lambda_hist,lagr_hist]=strength_reduction(alpha,...
...
%      
  ...
  % visualization of the curve: alpha -> lambda 
  figure
  hold on
  plot(lambda_hist, lagr_hist);
