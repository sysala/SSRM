...
%
%    output: 
%      f_V - vector of volume forces, size(f_V)=(3,n_n) where n_n is
%            the number of nodes
%    input data:
...
%                size(f_V_int)=(3,n_int), where n_int=n_e*n_q is a number
...
%
% Assembling of the vector of volume forces, size(f_V)=(3,n_n)
%
  % values at integration points, size(vF1)=size(vF2)=size(vF2)=(n_p,n_int)   
